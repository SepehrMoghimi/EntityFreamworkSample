...
public class StockRepository : IScopedService
{
    ...
    public async Task<List<Stock>> GetAllAsync(StockQuery queryObj)
    {
        ...
        if (!string.IsNullOrWhiteSpace(queryObj.CompanyName))
        {
            ...
            parameters.Add("@CompanyName", $"%{queryObj.CompanyName}%");
        }

        if (!string.IsNullOrWhiteSpace(queryObj.Symbol))
        {
            sqlQuery += " AND Symbol LIKE @Symbol";
            parameters.Add("@Symbol", $"%{queryObj.Symbol}%");
        }
        ...
        if (!string.IsNullOrWhiteSpace(queryObj.SortBy))
        {
            ...
            {
                sqlQuery += queryObj.IsDecsending ? " ORDER BY Symbol DESC" : " ORDER BY Symbol ASC";
            }
        }

        var skipNumber = (queryObj.PageNumber - 1) * queryObj.PageSize;
        sqlQuery += $" LIMIT {queryObj.PageSize} OFFSET {skipNumber}";
